      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '48px 24px' }}>
        ...
        <div style={{ marginBottom: 36 }}>
          ...
          <p style={{ fontSize: 15, color: '#5F5E5A', margin: 0, lineHeight: 1.6 }}>
            Claude generates personalised 80-120 word narratives for every student and packages them for download.
          </p>
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #E8E6DF', borderRadius: 16, padding: 28, marginBottom: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 24 }}>
            <div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#5F5E5A', letterSpacing: '0.05em', marginBottom: 7 }}>
                SELECT CLASS
              </label>
              <select
          ...
          <div style={{ background: '#F1EFE8', borderRadius: 8, padding: '12px 16px', display: 'flex', gap: 32, marginBottom: 24 }}>
            {[
              { label: 'Class', value: selClass.label },
              { label: 'Term', value: selTerm },
        ...
        {status === 'success' && reports.length > 0 && (
          <div style={{ background: '#fff', border: '1.5px solid #1D9E75', borderRadius: 16, padding: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {reports.map((r, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: '#F8F7F4', borderRadius: 8, border: '1px solid #E8E6DF' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 28, height: 28, borderRadius: 6, background: '#E1F5EE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: '#0F6E56' }}>
                      {i + 1}
                    </div>
                    <div>
            ...
            <div style={{ marginTop: 16, padding: '12px 16px', background: '#F1EFE8', borderRadius: 8, fontSize: 12, color: '#5F5E5A' }}>
              Tip: Open HTML file in Chrome → Ctrl+P → Save as PDF
            </div>
          </div>
        )}
...
function Spinner() {
  return (
    <>
      <style>{`@keyframes schoolos_spin{to{transform:rotate(360deg)}}`}</style>
      <div style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,0.35)', borderTop: '2px solid #fff', borderRadius: '50%', animation: 'schoolos_spin 0.7s linear infinite' }} />
    </>
  );
}
