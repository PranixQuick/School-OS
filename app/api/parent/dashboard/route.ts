...
export async function GET(req: NextRequest) {
  try {
    ...
    const { schoolId, parentId, studentId } = session;

    const [studentRes, attRes, feeRes, noticesRes, schoolRes] = await Promise.allSettled([
      supabaseAdmin.from('students').select('name,class,section,roll_number,admission_number').eq('id', studentId).single(),
      supabaseAdmin.from('attendance').select('status').eq('school_id', schoolId).eq('student_id', studentId).gte('date', new Date(Date.now() - 90 * 86400000).toISOString().split('T')[0]),
      supabaseAdmin.from('fees').select('amount,status').eq('school_id', schoolId).eq('student_id', studentId).in('status', ['pending','overdue']),
      supabaseAdmin.from('broadcasts').select('id,subject,message,created_at').eq('school_id', schoolId).order('created_at', { ascending: false }).limit(5),
