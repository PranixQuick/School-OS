...
export async function GET() {
  try {
    const [
      studentsRes, staffRes, feesRes,
      leadsRes, recordingsRes, eventsRes, narrativesRes,
    ] = await Promise.all([
      ...
      supabaseAdmin.from('staff').select('id', { count: 'exact', head: true })
        .eq('school_id', SCHOOL_ID).eq('is_active', true),
      supabaseAdmin.from('fees').select('id, status, amount', { count: 'exact' })
        .eq('school_id', SCHOOL_ID).in('status', ['pending', 'overdue']),
      ...
      supabaseAdmin.from('events')
        ...
        .limit(4),
      supabaseAdmin.from('report_narratives')
    ...
    return NextResponse.json({
      kpis: {
        ...
        evals_done: recordingsRes.data?.length ?? 0,
