...
// runImport() is exported so connectors/sheets/route.ts can call it directly
// without making an internal HTTP fetch (which would be blocked by middleware).
...
// ─── Core import runner — exported for direct use by sheets/route.ts ─────────
    ...
    try {
      if (entity === 'students') {
        ...
        const { error } = await supabaseAdmin.from('attendance').upsert({
          school_id: schoolId, student_id: sid,
          date: n.date, status: n.status, marked_via: n.marked_via, data_source: source,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = await req.json() as {
      source?: DataSource; entity: Entity;
      data: Record<string, unknown>[];
      filename?: string; sheet_url?: string; dry_run?: boolean;
    };
