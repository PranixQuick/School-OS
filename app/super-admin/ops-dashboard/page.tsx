      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Super-admin · Auto-refreshes every 60s · {new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} IST
          </p>
        </div>
        <span className={`px-4 py-2 rounded-full text-sm font-semibold border ${HC[overallHealth]}`}>
          {overallHealth === 'green' ? '✅ All systems operational' : overallHealth === 'yellow' ? '⚠️ Degraded' : '🔴 Action required'}
        </span>
      </div>
      ...
      <div className="grid grid-cols-5 gap-3 mb-8">
        {summary.map((row: any) => (
          <div key={row.panel} className={`rounded-lg border p-3 ${HC[row.health]}`}>
            <div className="text-xs font-medium uppercase tracking-wide opacity-70">{row.panel.replace(/_/g, ' ')}</div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-left text-gray-400 text-xs uppercase">
              <th className="pb-2">Job</th><th>✅</th><th>❌</th><th>Last success</th><th>Status</th>
            </tr></thead>
            <tbody>
              {cronHealth.map((row: any) => (
                <tr key={row.job_name} className="border-t border-gray-50">
                  ...
                  <td className="text-gray-400 text-xs">{row.last_success ? new Date(row.last_success).toLocaleDateString('en-IN') : 'never'}</td>
                  <td><span className={`px-2 py-0.5 rounded text-xs ${HC[row.health]}`}>{row.health}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          {failedNotifs.length === 0
            ...
            : <table className="w-full text-sm">
                <thead><tr className="text-left text-gray-400 text-xs uppercase">
                  <th className="pb-2">Module</th><th>Stuck</th><th>&gt;24h</th><th>Status</th>
                </tr></thead>
                <tbody>
                  {failedNotifs.map((row: any) => (
                    <tr key={row.module} className="border-t border-gray-50">
                      ...
                      <td className={row.older_than_24h > 0 ? 'text-red-600 font-semibold' : '}>{row.older_than_24h}</td>
                      <td><span className={`px-2 py-0.5 rounded text-xs ${HC[row.health]}`}>{row.health}</span></td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          {systemHealth.map((row: any) => (
            <div key={row.service} className="flex items-center justify-between mb-2">
              <div>
                <span className="text-sm font-medium capitalize">{row.service.replace(/_/g, ' ')}</span>
                {row.error_message && <p className="text-xs text-gray-400 mt-0.5">{row.error_message}</p>}
              </div>
              <span className={`px-2 py-0.5 rounded text-xs border ${HC[row.status]}`}>{row.status}</span>
            </div>
          ))}
        </div>
      </div>
