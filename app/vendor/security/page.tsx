'use client';
// app/vendor/security/page.tsx
// ISS-1 (#1 / P4.6) — Vendor self-service PIN change.
// Posts to /api/vendor/change-pin. Redirects to /vendor/login on an expired session.
...
export default function VendorSecurityPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const r = await fetch('/api/vendor/change-pin', {
        method: 'POST',
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', paddingBottom: 40 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1E40AF,#0EA5E9)', padding: '16px 16px 20px' }}>
        <a href="/vendor" style={{ color: 'rgba(255,255,255,0.9)', fontSize: 13, textDecoration: 'none', fontWeight: 600 }}>← Back</a>
        <div style={{ color: '#fff', fontSize: 20, fontWeight: 800, marginTop: 10 }}>Change PIN</div>
        <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13, marginTop: 2 }}>Update your login PIN</div>
      </div>
      ...
      <div style={{ padding: 16, maxWidth: 440, margin: '0 auto' }}>
        {done ? (
          <div style={{ background: '#ECFDF5', border: '1px solid #A7F3D0', borderRadius: 14, padding: 20, color: '#065F46' }}>
            ...
            <a href="/vendor" style={{ display: 'inline-block', marginTop: 14, background: '#fff', color: '#065F46', border: '1px solid #A7F3D0', borderRadius: 10, padding: '9px 16px', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>
              Back to portal
            </a>
          </div>
        ) : (
          <form onSubmit={submit} style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 14, padding: 18, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button
              type="submit"
              disabled={busy}
              style={{ background: busy ? '#9CA3AF' : '#1E40AF', color: '#fff', border: 'none', borderRadius: 10, padding: '13px 18px', fontSize: 15, fontWeight: 700, cursor: busy ? 'wait' : 'pointer' }}
            >
