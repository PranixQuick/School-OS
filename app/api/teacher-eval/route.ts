...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from('recordings')
      .select('id, staff_id, file_name, transcript, eval_report, coaching_score, status, uploaded_at, processed_at')
      .eq('school_id', session.schoolId)
      .eq('staff_id', session.userId) // Limit to current teacher's evaluations
      .is('deleted_at', null)
    ...
    const mapped = (data ?? []).map(r => {
      let reportStr = '';
      if (r.eval_report) {
        if (typeof r.eval_report === 'string') {
          try {
            const parsed = JSON.parse(r.eval_report);
            reportStr = parsed.feedback ?? parsed.error ?? parsed.detail ?? r.eval_report;
          } catch {
            reportStr = r.eval_report;
          }
        } else {
          const rep = r.eval_report as any;
          reportStr = rep.feedback ?? rep.error ?? rep.detail ?? '';
        }
      }
      return {
        id: r.id,
        file_name: r.file_name,
        coaching_score: r.coaching_score,
        status: r.status,
        uploaded_at: r.uploaded_at,
        eval_report: reportStr,
      };
    });

    return NextResponse.json({ evals: mapped });
  } catch (err) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const teacherName = staffData?.name || session.userName || 'Teacher';
    ...
    // Dual-write institution_id alongside school_id.
