'use client';
// app/parent/security/page.tsx
// ISS-1 (#1 / P4.6) — Parent self-service PIN change.
// Posts to /api/parent/change-pin. On an unauthenticated response, sends the
// parent to /parent/login.
...
export default function ParentSecurityPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (r.status === 401 && String(d.error ?? '').toLowerCase().includes('signed in')) {
      ...
      <div style={{ padding: 16, maxWidth: 440, margin: '0 auto' }}>
        {done ? (
          <div style={{ background: '#ECFDF5', border: '1px solid #A7F3D0', borderRadius: 14, padding: 20, color: '#065F46' }}>
            ...
            <Link href="/parent" style={{ display: 'inline-block', marginTop: 14, background: '#fff', color: '#065F46', border: '1px solid #A7F3D0', borderRadius: 10, padding: '9px 16px', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>
              Back to home
            </Link>
          </div>
        ) : (
