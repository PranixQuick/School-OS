...
export default function TeacherAttendancePage() {
  ...
  return (
    <Layout title="Teacher Attendance" subtitle="Daily staff attendance marking"
      actions={<div style={{ display: 'flex', gap: 8 }}><Link href="/automation" className="btn btn-ghost btn-sm">← Automation</Link></div>}
    >
      <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
        <div>
          <label className="label">DATE</label>
          <input type="date" className="input" style={{ width: 180, height: 38 }} value={date} onChange={e => setDate(e.target.value)} max={today} />
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, auto)', gap: 10, marginTop: 20 }}>
          {[{ l: 'Present', v: summary.present, c: '#15803D', bg: '#DCFCE7' }, { l: 'Absent', v: summary.absent, c: '#B91C1C', bg: '#FEE2E2' }, { l: 'Late', v: summary.late, c: '#A16207', bg: '#FEF9C3' }, { l: 'Unmarked', v: summary.not_marked, c: '#9CA3AF', bg: '#F3F4F6' }].map(k => (
        ...
        {summary.not_marked > 0 && (
          <button onClick={markAllPresent} disabled={bulkMarking} className="btn btn-ghost btn-sm" style={{ marginTop: 20, background: '#DCFCE7', borderColor: '#BBF7D0', color: '#15803D' }}>
      ...
      {staff.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 13 }}>
            ...
            <span style={{ fontWeight: 700, color: presentPct >= 80 ? '#15803D' : presentPct >= 60 ? '#A16207' : '#B91C1C' }}>{presentPct}%</span>
          </div>
          <div className="progress-bar">
            <div className="progress-fill" style={{ width: `${presentPct}%`, background: presentPct >= 80 ? '#22C55E' : presentPct >= 60 ? '#F59E0B' : '#EF4444' }} />
          </div>
        </div>
      )}
      ...
      <div className="table-wrapper">
        <table className="table">
          <thead>
            <tr>
              <th>Teacher</th><th>Role / Subject</th><th>Check-in</th><th>Status</th><th>Mark Attendance</th>
            </tr>
          </thead>
          <tbody>
            {staff.map(s => (
              <tr key={s.id}>
                ...
                <td>
                  <div style={{ fontSize: 13, color: '#374151', fontWeight: 500 }}>{s.role.charAt(0).toUpperCase() + s.role.slice(1)}</div>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 6 }}>
                    {STATUS_OPTIONS.map(status => (
                      <button key={status} onClick={() => markAttendance(s.id, status)} disabled={saving === s.id}
                        style={{ height: 28, padding: '0 10px', borderRadius: 6, border: `1px solid ${s.status === status ? STATUS_COLOR[status] : '#E5E7EB'}`, background: s.status === status ? STATUS_COLOR[status] + '15' : '#fff', color: s.status === status ? STATUS_COLOR[status] : '#9CA3AF', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: saving === s.id ? 0.5 : 1 }}>
      ...
      <div style={{ marginTop: 14, display: 'flex', gap: 14, fontSize: 12, color: '#9CA3AF', flexWrap: 'wrap' }}>
        {STATUS_OPTIONS.map(s => (
          <span key={s} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ width: 18, height: 18, borderRadius: 4, border: '1px solid', borderColor: STATUS_COLOR[s], background: STATUS_COLOR[s] + '15', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 10, color: STATUS_COLOR[s] }}>
