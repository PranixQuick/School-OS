...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.name || !body.class) return NextResponse.json({ error: 'name and class are required' }, { status: 400 });
    ...
    if (body.phone_parent) {
      ...
      }, { onConflict: 'school_id,student_id' }).then(null, (e: unknown) => { console.error('Parent record error:', e); });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { error } = await supabaseAdmin.from('students').update({ is_active: false }).eq('id', id).eq('school_id', schoolId);
    if (error) throw new Error(error.message);
