...
type ValidRow = { name: string; role: string; email: string | null; phone: string | null; subject: string | null; designation: string | null; joined_at: string | null; department: string | null; scope_schools: string[] | null };
...
function validateRow(raw: RawRow, index: number): { ok: true; value: ValidRow } | { ok: false; error: RowError } {
  ...
  const scopeSchoolsRaw = str(raw.scope_schools) || str(raw.scope_school) || '';
  const scope_schools = scopeSchoolsRaw ? scopeSchoolsRaw.split(',').map(s => s.trim()).filter(Boolean) : null;
  return { ok: true, value: { name, role, email, phone, subject, designation, joined_at, department, scope_schools } };
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const schoolSlugMap = new Map(allSchools?.map(s => [s.slug?.toLowerCase(), s.id]) ?? []);
    ...
    const { data: inserted, error: insErr } = await supabaseAdmin.from('staff').insert(payload).select('id, name, email');
    if (insErr) throw new Error(`insert failed: ${insErr.message}`);

    const staffEmailMap = new Map(inserted?.map(x => [x.email?.toLowerCase(), x.id]) ?? []);
    const staffNameMap = new Map(inserted?.map(x => [x.name.toLowerCase(), x.id]) ?? []);
    const scopePayload: any[] = [];
    
    for (const v of toInsert) {
      const staffId = v.email ? staffEmailMap.get(v.email) : staffNameMap.get(v.name.toLowerCase());
      if (!staffId) continue;
      if (v.role.toLowerCase() === 'hod' && v.department) {
        const schoolCodes = v.scope_schools ?? [];
        const schoolIds = schoolCodes.map(code => schoolSlugMap.get(code.toLowerCase())).filter((id): id is string => !!id);
        for (const sid of schoolIds) {
          scopePayload.push({ staff_id: staffId, school_id: sid, department: v.department.toLowerCase() });
        }
      }
    }
    
    if (scopePayload.length > 0) {
      const { error: scopeErr } = await supabaseAdmin.from('staff_hod_scope').insert(scopePayload);
      if (scopeErr) throw new Error(`HOD scope insert failed: ${scopeErr.message}`);
    }
