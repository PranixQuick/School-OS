// app/api/student/login-otp/request/route.ts
// ISS-OTP PR6 (students) — passwordless login: request a login OTP (to parent phone).
...
export async function POST(req: NextRequest) {
  ...
  let q = supabaseAdmin
    .from('students').select('id, school_id, phone_parent')
  ...
  const st = matches?.[0];
  if (!st || !st.phone_parent) return generic;
  ...
  if ((count ?? 0) < MAX_PER_PHONE) {
    await requestOtp({ phone, purpose: 'login', schoolId: st.school_id });
  } else {
    console.warn('[student/login-otp] rate-limited', maskPhone(phone));
  }
