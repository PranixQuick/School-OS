...
async function trackRun(params: {
  ...
  // Concurrent-run guard: if this job is already running for this school
  // within the last 5 minutes, skip to prevent double-execution
  ...
  if (existing) {
    console.log(
      `[Cron] Skipping ${params.jobName} for ${params.schoolId} — already running since ${existing.started_at}`
    );
    return ''; // empty string signals callers to skip
  }
...
async function completeRun(runId: string, params: {
  ...
  if (!runId) return; // was skipped due to concurrent run — nothing to update
...
export async function runFeeReminders(
  ...
  if (!runId) {
    return { job: 'fee_reminders', schoolId: school.id, success: true, data: { skipped: true, reason: 'Already running' }, durationMs: 0 };
  }
...
School: ${school.name}. Include a friendly payment request and contact number 040-12345678.`,
        ...
        // Upsert with ignoreDuplicates — safe against race conditions
...
export async function runRiskDetection(
  ...
  // Concurrent run detected — return early
  if (!runId) {
    return { job: 'risk_detection', schoolId: school.id, success: true, data: { skipped: true, reason: 'Already running' }, durationMs: 0 };
  }
...
export async function runPrincipalBriefing(
  ...
  // Concurrent run detected — return early
  if (!runId) {
    return { job: 'principal_briefing', schoolId: school.id, success: true, data: { skipped: true, reason: 'Already running' }, durationMs: 0 };
  }
