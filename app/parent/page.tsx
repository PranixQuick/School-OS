...
const QUICK_ACTIONS = [
  { href: '/parent/attendance', icon: '✅', label: 'Attendance', color: '#16A34A', bg: '#F0FDF4' },
  { href: '/parent/homework', icon: '📚', label: 'Homework', color: '#9333EA', bg: '#FDF4FF' },
  { href: '/parent/fees', icon: '💳', label: 'Fees', color: '#D97706', bg: '#FFFBEB' },
  { href: '/parent/marks', icon: '📊', label: 'Marks', color: '#0284C7', bg: '#F0F9FF' },
  { href: '/parent/timetable', icon: '🗓️', label: 'Timetable', color: '#4F46E5', bg: '#EEF2FF' },
  { href: '/parent/notices', icon: '📢', label: 'Notices', color: '#374151', bg: '#F9FAFB' },
  { href: '/parent/events', icon: '📸', label: 'Events', color: '#9333EA', bg: '#FDF4FF' },
];
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', paddingBottom: 40 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #4F46E5 0%, #7C3AED 100%)', padding: '20px 16px 28px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <button onClick={handleLogout} style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, color: '#fff', cursor: 'pointer', fontWeight: 600 }}>Sign Out</button>
        </div>
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)' }}>Parent Portal</div>
      </div>
      ...
      <div style={{ padding: '0 16px', marginTop: -16 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 20, marginBottom: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
          {loading ? (
            ...
            <>
              ...
              <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 14 }}>Class {student.class}-{student.section}{student.roll_number ? ` · Roll No. ${student.roll_number}` : ''}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div style={{ background: '#F9FAFB', borderRadius: 12, padding: '12px 14px', borderLeft: `3px solid ${attColor}` }}>
                  ...
                  <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>Attendance</div>
                ...
                <div style={{ background: feeAlert ? '#FEF2F2' : '#F0FDF4', borderRadius: 12, padding: '12px 14px', borderLeft: `3px solid ${feeAlert ? '#B91C1C' : '#16A34A'}` }}>
                  <div style={{ fontSize: 22, fontWeight: 900, color: feeAlert ? '#B91C1C' : '#16A34A' }}>
                    {feeAlert ? `₹${fee!.pending_amount >= 1000 ? (fee!.pending_amount / 1000).toFixed(1) + 'K' : fee!.pending_amount}` : '✓ Clear'}
                  </div>
                  <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>{feeAlert ? 'Fees due' : 'Fees paid'}</div>
        ...
        {!loading && feeAlert && (
          <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 12, padding: '12px 16px', marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontWeight: 700, fontSize: 13, color: '#B91C1C' }}>⚠️ Fee payment pending</div>
              <div style={{ fontSize: 12, color: '#B91C1C', marginTop: 2 }}>₹{fee!.pending_amount} outstanding{fee!.overdue ? ' — overdue' : ''}</div>
            </div>
            <Link href="/parent/fees" style={{ padding: '7px 14px', background: '#B91C1C', color: '#fff', borderRadius: 8, fontSize: 12, fontWeight: 700, textDecoration: 'none' }}>Pay Now →</Link>
          </div>
        )}

        {/* Quick actions — 4 wide, then 3 wide */}
        ...
        {/* School notices */}
        <div style={{ fontSize: 12, fontWeight: 700, color: '#9CA3AF', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 10 }}>School Notices</div>
        {loading ? (
